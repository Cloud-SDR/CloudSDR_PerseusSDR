...
// ------------------------------------------------------------------------------
// ported to Qt 5.xx Linux & Windows 32/64 bits by Sylvain AZARIAN - F4GKR
//-------------------------------------------------------------------------------
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int	perseus_init(void)
{
...
#elif defined _WIN64
    {
        ...
        if (GetModuleFileName(NULL, (LPWSTR)myPath, sizeof(myPath)) == 0) {
    ...
    for (i=0;i<num_devs;i++) {
        ...
        if (descr.idVendor==PERSEUS_VID) {
            if (descr.idProduct==PERSEUS_PID_BLANKEEPROM) {
                ...
                printf("Found PERSEUS_PID_BLANKEEPROM device with VID/PID %04X:%04X on BUS%d ADDR%d\n",
                       descr.idVendor,
                       descr.idProduct,
                ...
                if (descr.idProduct==PERSEUS_PID) {
                    ...
                    printf("Found PERSEUS_PID device with VID/PID %04X:%04X on BUS%d ADDR%d\n",
                           descr.idVendor,
                           descr.idProduct,
                           perseus_list[num_perseus].bus,
                           perseus_list[num_perseus].devaddr);
       ...
       if ((num_perseus+1)==PERSEUS_MAX_DESCR)
            break;
    }
                              ...
                              void *cb_extra)
{
    ...
    if ((rc=perseus_fx2_sio(descr->handle, &descr->sioctl, NULL))<0) {
        ...
        return errorset(PERSEUS_ASYNCSTARTED, "FPGA fifo enable failed");
    }
    return errornone(0);
}
