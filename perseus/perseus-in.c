					...
					void *callback_extra)
{
    ...
    //printf("perseus_input_queue_create()\n") ;
    ...
    queue->timeout			 = 0 ; //queue_size*PERSEUS_INPUT_TIMEOUT;
	...
	for (k=0; k<queue_size; k++) {
        ...
        if (!transfer_queue[k].transfer) {
            printf("libusb_alloc_transfer() %d error\n") ;
			goto create_cleanup;
        }
    ...
    //printf("submit transfers\n") ;
	// submit the transfers
    for (k=0; k<queue_size; k++) {
        error = libusb_submit_transfer(transfer_queue[k].transfer);
        if( error != 0 ) {
            return errorset(PERSEUS_IOERROR,"can't submit transfer %d", k);
        }
    }
